...
router.post("/post", verifyAdmin, async (req, res) => {
  const { name, chargerType, chargerPower, chargerAvailability, coordinates } =
    req.body;
  ...
  if (
    ...
    return res.status(400).send({
      message:
        "All fields (id, name, chargerType, chargerPower, chargerAvailability, coordinates) are required",
    });
  }
...
router.put("/put/availability", verifyAdmin, async (req, res) => {
  ...
  if (!stationId || !availability) {
    return res
      .status(400)
      .send("Request must have station id and availability");
  }
  ...
  try {
    ...
    if (updatedStation) res.status(200).json(updatedStation);
    else
      res.status(404).json({
