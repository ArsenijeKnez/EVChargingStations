...
const verifyUser = require('./JWTverification/VerifyUser');
  ...
  router.get('/bestRoute', verifyUser, async (req, res) => {
    ...
    const stations = await Station.find({ chargerType: car.chargerType, chargerAvailability: "Available"});

    if (!stations.length) {
      return res.status(404).json({ message: "No station found with apropriate charger" });
    }
    ...
    for (const station of stations) {
      ...
      if (!nearestStation) {
          return res.status(404).send({ message: 'No suitable charging station found' });
      }
